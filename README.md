...
2. **가격 데이터**: 선택한 조회 기간(기본 13개월, 최대 10년)만큼 일별 종가를 배치 다운로드 (yfinance).
   모멘텀 점수는 항상 최근 13개월 데이터로 계산되며, 기간을 늘리면 백테스트에서 더 과거 날짜까지 선택 가능
...
6. **간단 백테스트**: 선정된 K개 종목을 스크리닝 기간(약 13개월) 동안 동일가중으로 매수·보유했다고
   가정할 때의 CAGR·MDD·최종 평가액 계산 (리밸런싱·거래비용 미반영, 참고용 approximation)
